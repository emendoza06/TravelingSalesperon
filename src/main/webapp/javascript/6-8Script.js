...
function draw() {
  ...
  /*-----------After Animation Runs (User Interaction)-----------------------*/
    ...
    for (var acc = 0; acc < shortestRoute.length - 1; acc++) {
      shortestRouteTotalDistance += dist(
        shortestRoute[acc].x,
        shortestRoute[acc].y,
    ...
    $("#startingnode").html(
      "Draw the shortest and longest routes starting and ending on point  " +
...
function buildLongestRoute() {
  ...
  for (var i = 0; i < shortestRouteCopy.length - 1; i++) {
    var current_dist = dist(
      shortestRouteCopy[0].x,
      shortestRouteCopy[0].y,
      shortestRouteCopy[i + 1].x,
      shortestRouteCopy[i + 1].y,
    );
  ...
  for (var acc = 0; acc < longestRoute.length - 1; acc++) {
    longestRouteTotalDistance += dist(
      longestRoute[acc].x,
      longestRoute[acc].y,
      longestRoute[acc + 1].x,
      longestRoute[acc + 1].y,
    );
  }
